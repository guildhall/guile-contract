(define-module (ice-9 contract src parametric)
...
(define-rstruct polymorphic-contract [title barrier vars body]
  ...
  (build-contract-property
   ...
   (lambda (c)
     (lambda (b)
       ...
       (lambda (p)
         (unless (procedure? p)
           (raise-blame-error b p "expected a procedure; got: ~s" p))
...
(define-rstruct barrier-contract [name positive? make pred get]
  #:property prop:contract
  (build-contract-property
       ...
       (if (equal? (blame-original? b) (barrier-contract-positive? c))
         ...
         (lambda (x)
           (if ((barrier-contract-pred c) x)
             ...
             (raise-blame-error b x "expected a(n) ~a; got: ~s"
