(define-module (ice-9 contract src struct)
...
(define-syntax struct/c 
  (lambda (stx)
    (syntax-case stx ()
      [(_ . args) 
       (with-syntax ([x (syntax (do-struct/c . args))])
         (syntax (%expression x)))])))
...
(define-rstruct base-struct/c (name predicate immutables mutables))

(define (struct/c-name ctc)
  (let ([ctcs (map second
                   (sort (append (base-struct/c-immutables ctc) 
                                 (base-struct/c-mutables ctc))
...
(define (check-struct/c ctc)
  (let ([name  (base-struct/c-name ctc)]
        ...
        [ctc/ref-pairs 
         (map (λ (l) (cons (second l) (third l)))
              (append (base-struct/c-immutables ctc) 
                      (base-struct/c-mutables ctc)))])
    (lambda* (val fail #:optional [first-order? #f])
      (unless (pred? val)
        (rerror #f "expected <~a>, got ~a" name val))
      (when first-order?
        (for ([p (in-list ctc/ref-pairs)])
          (let ([c (car p)] [v ((cdr p) val)])
            (unless (contract-first-order-passes? c v)
              (rerror #f "expected <~a>, got ~a" (contract-name c) v)))))
      #t)))
...
(define (struct/c-first-order ctc)
  (let ([f (check-struct/c ctc)])
    (λ (val)
      (let/ec fail
        (f val (λ args (rerror #f "struct first order")) #t)))))

(define (flat-struct/c-proj ctc)
  (let ([checker (check-struct/c ctc)]
        [name (base-struct/c-name ctc)]
        [pred (base-struct/c-predicate ctc)]
        [projs (map contract-projection 
                    (map second (base-struct/c-immutables ctc)))]
...
(define-rstruct (flat-struct/c base-struct/c) ()
...
(define (chaperone-struct/c-proj ctc)
  (let-values ([(flat-imms chap-imms)
                (partition (λ (l) (flat-contract? (second l))) 
                           (base-struct/c-immutables ctc))])
    (let ([checker (check-struct/c ctc)]
          [name (base-struct/c-name ctc)]
          [pred (base-struct/c-predicate ctc)]
          ...
          [mut-projs (map (compose contract-projection second) 
                          (base-struct/c-mutables ctc))]
      ...
      (λ (blame)
        (let* ([swapped-blame (blame-swap blame)]
          ...
          (λ (val)
            ...
            ;; we go ahead and apply the projection to check the first order 
            ;; properties.
            (let ([combined-imm-refs 
              ...
              (apply chaperone-struct val
                     (flatten (list combined-imm-refs combined-mut-refs 
                                    combined-mut-sets
                                    impersonator-prop:contracted ctc))))))))))

(define-rstruct (chaperone-struct/c base-struct/c) ()
...
(define-rstruct (impersonator-struct/c base-struct/c) ()
    ...
    [(_ struct-name args ...)
     ...
     (let* ([si (extract-struct-info (syntax-local-value (syntax struct-name)))]
       ...
       (unless (= (length selector-ids) (length ctcs))
         (raise-syntax-error 
          'struct/c 
          (format #f "expected ~a contracts because struct ~a has ~a fields"
                  (length selector-ids)
       ...
       (let ([combined-ids (for/list ([n        (in-naturals)]
         ...
         (let-values ([(mutables immutables) 
                       (partition (λ (l) (fourth l)) combined-ids)])
             ...
             (syntax
              (let ([ctc-x (coerce-contract 'struct/c args)] ...)
                ...
                (unless (chaperone-contract? imm-ctc-x)
                  (rerror 
                   'struct/c 
                   (string-append "expected a chaperone contract for immutable "
                                  "field ~v, got ~a")
                   imm-count imm-ctc-x))
                ...
                
                (let ([immutables 
                       (list (list imm-count imm-ctc-x imm-ref) ...)]
                      [mutables 
                       (list (list mut-count mut-ctc-x mut-ref mut-set) ...)])
                  (cond
                    [(and (null? mutables) 
                          (andmap (λ (l) 
                                     (flat-contract? (second l))) immutables))
                     (make-flat-struct/c 'struct-name predicate-id immutables
                                         mutables)]
                    [(andmap (λ (l) (chaperone-contract? (second l))) mutables)
                     (make-chaperone-struct/c 'struct-name predicate-id 
                                              immutables mutables)]
                    [else
                     (make-impersonator-struct/c 'struct-name predicate-id 
                                                 immutables mutables)]))))))))]
    [(_ struct-name anything ...)
     (raise-syntax-error 'struct/c "expected a struct identifier" stx 
                         (syntax struct-name))])))
