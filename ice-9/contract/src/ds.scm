(define-module (ice-9 contract src ds)
  #:use-module (ice-9 contract src guts)
  #:use-module (ice-9 contract src opt)
  #:use-module (ice-9 contract src ds-helpers)
...
(define (main stx name fields inspector define-struct?)
  (with-syntax ([name name]
    ...
    (let* ([mutable? (syntax-e (syntax mutable?))]
              ...
              (datum->syntax 
               ...
               (string->symbol 
                (string-append (symbol->string (syntax-e (syntax name)))
                               suffix))))]
           [struct-names 
            (build-struct-names (syntax name)
      ...
      (with-syntax ([struct/c        struct/c-name/val]
                    ...
                    [(selector-indices ...) 
                     (nums-up-to field-count/val)]

                    [(selector-indices+1 ...) 
                     (map (lambda (x) (+ x 1)) (nums-up-to field-count/val))]

                    [(ctc-x ...)      (generate-temporaries 
                                       (syntax (fields ...)))]
                    [(f-x ...)        f-x/vals]
                    [((f-xs ...) ...) f-xs/vals]
                    [wrap-name        (datum->syntax
                                       struct:-name/val
                                       (string->symbol 
                                        (format #f "~a/lazy-contract" 
                                                (syntax-e (syntax name)))))]
                    [opt-wrap-name 
                     (string->symbol 
                      (format #f "~a/lazy-opt-contract" 
                              (syntax-e (syntax name))))])
                    
          #`(begin          
              ;; `declare' future bindings for the top-level (so that everyone 
              ;;    picks them up)
              #|#,@(if (eq? (syntax-local-context) 'top-level)
                     (list
                      (syntax
                       (define-syntaxes 
                         (contract-type contract-maker contract-predicate 
                                        contract-get contract-set 
                                        already-there? burrow-in 
                                        rewrite-fields wrap-get)
                         (values))))
              (list))|#
                              ...
                              (if (identifier? stx)
                                  ...
                                  (let ([pr (syntax-e stx)])
                                    (if (pair? pr)
                                        (datum->syntax 
                                         #'here 
                                         (cons #'struct-maker (cdr pr)))
                                        (raise-syntax-error 
                                         #f "what happened?" stx)))))))))
          
          (define (evaluate-attrs stct contract/info)
            (when (wrap-parent-get stct 0) 
              ;; test to make sure this even has attributes
              (let* ([any-unknown?      #f]
                     [any-became-known? #f]
                ...
                (when any-became-known?
                  (for-each 
                   (lambda (x) ((evaluate-attr-prop-accessor x) 
                                x contract/info))
          ...
          (define-values (opt-wrap-type opt-wrap-maker 
                                        opt-wrap-predicate 
                                        opt-wrap-get opt-wrap-set)
          ...
          (define (predicate x) (or (raw-predicate x) 
                                    (opt-wrap-predicate x) 
                                    (wrap-predicate x)))
          
          (define-syntax struct/dc 
            (lambda (stx)
            (syntax-case stx ()
              [(_ clause (... ...))
                               ...
                               (let ((inner (opt-wrap-get stct 0)))
                                 (if inner
                                     (let* ((info (opt-wrap-get stct 1))
                                            (enforcer 
                                             (opt-contract/info-enforcer info)))
                                       (let-values 
                                           ([(inner-stct fields ...) 
                                             (loop inner)])
                                         (let-values 
                                             ([(fields ...) 
                                               (enforcer stct fields ...)])
                                           (opt-wrap-set stct 0 #f)
                                           (opt-wrap-set 
                                            stct selector-indices+1 fields) 
                                           ...
                                           (values stct fields ...))))
                                     
                                     ;; found a cached version
                                     (values #f (opt-wrap-get 
                                                 stct 
                                                 selector-indices+1) ...)))]
                              [(wrap-predicate stct)
                               (let ([inner (wrap-get stct 0)])
                                 (if inner
                                     ...
                                     (let ([contract/info (wrap-get stct 1)])
                                       (let-values 
                                           ([(_1 fields ...) (loop inner)])
                                         (let-values 
                                             ([(fields ...) 
                                               (rewrite-fields 
                                                stct contract/info fields ...)])
                                           (wrap-set stct 0 #f)
                                           (wrap-set stct selector-indices+1 
                                                     fields) ...
          ...
          (define (rewrite-fields parent contract/info ctc-x ...)
            (let* ([f-x (let* ([ctc-field (contract-get 
                     ...
                     (let ([wrapper (wrap-maker val contract/info)])
                       (let ([synth-setup-stuff (contract-get ctc field-count)])
                         (when synth-setup-stuff
                           (let ([ht (make-hash)])
                             (for-each (λ (pr) (hash-set! 
                                                ht (car pr) 
                                                (make-unknown (cdr pr))))
                                       (cdr synth-setup-stuff))
                             (wrap-parent-set wrapper 0 
                                              (make-synth-info 
                                               '() ht 
                                               (car synth-setup-stuff))))))
                       wrapper)])))))
               ...
               (and (wrap-get val 0)
                    (let ([old-contract/info (wrap-get val 1)])
                      (if (and (equal? (contract/info-blame new-contract/info)
                               ...
                               (contract-stronger? 
                                (contract/info-contract old-contract/info)
                                (contract/info-contract new-contract/info)))
                          #t
                          (already-there? 
                           new-contract/info (wrap-get val 0) (- depth 1)))))]
              [else 
            ...
            (cond
              ...
              [(wrap-predicate struct)
               (if (wrap-get struct 0)
                   ...
                   (wrap-get struct (+ i 1)))]
              [else
               (error selector-name "expected <~a>, got ~a" 'name struct)]))
          ...
          (define-values (contract-type contract-maker contract-predicate 
                                        contract-get contract-set)
            (make-struct-type 'the-contract
                              #f
                              (+ field-count 1) 
                              ;; extra field is for synthesized attribute ctcs
                              ;; it is a list whose first element is
                              ;; a procedure (called once the attrs are known) 
                              ;; that indicates if the test passes. the rest 
                              ;; of the elements are procedures that build the 
                              ;; attrs this field is #f when there is no 
                              ;; synthesized attrs
                              0 ;; auto-field-k
                              '() ;; auto-field-v
                              (list (cons prop:contract 
                                          lazy-contract-property))))
          
          (define (build-enforcer opt/i opt/info name stx clauses
            ...
            (let*-values ([(inner-val) #'val]
              ...
              (with-syntax ([(clause (... ...)) clauses]
                            ...
                            [((free-var free-var-val) (... ...))
                             (make-free-vars (append (opt/info-free-vars 
                                                      opt/info)) #'freev)]
          ...
          (define/opter (struct/dc opt/i opt/info stx)
            (syntax-case stx ()
              [(_ clause (... ...))
               (let ((enforcer-id-var 
                      (car (generate-temporaries (syntax (enforcer)))))

                     (helper-id-var 
                      (car (generate-temporaries (syntax (helper)))))

                     (contract/info-var 
                      (car (generate-temporaries (syntax (contract/info)))))
                 ...
                 (let-values ([(enforcer lifts superlifts stronger-ribs)
                   ...
                   (let ([to-save (append (opt/info-free-vars opt/info)
                     ...
                     (with-syntax ((val (opt/info-val opt/info))
                       ...
                       (with-syntax 
                           ([(stronger-this-var (... ...)) 
                             (map stronger-rib-this-var stronger-ribs)]

                            [(stronger-that-var (... ...)) 
                             (map stronger-rib-that-var stronger-ribs)]

                            [(stronger-exps (... ...)) 
                             (map stronger-rib-stronger-exp stronger-ribs)]

                            [(stronger-indexes (... ...)) 
                             (build-list (length stronger-ribs) 
                                         (λ (x) (+ x 2)))]

                            [(stronger-var (... ...)) 
                             (map stronger-rib-save-id stronger-ribs)])
                         
                         (let ([partials
                                (list 
                                 (cons id-var #'(begin-lifted (box 'identity)))
                               ...
                               [(opt-wrap-predicate val)
                                (if (and 
                                     ...
                                     (let ([stronger-this-var stronger-var] 
                                           ...
                                           ;; it only works if the stronger 
                                           ;; vars and the things
                                           ;; saved in the wrapper are the same
                                           [stronger-that-var 
                                            (opt-wrap-get val stronger-indexes)]
                                           (... ...))
                                       (and 
                                        ;; make sure this is the same contract
                                        ;; -- if not,
                                        ;; the rest of this test is bogus and 
                                        ;; may fail at runtime
                                        (eq? id (opt-contract/info-id 
                                                 (opt-wrap-get val 1)))
                                        stronger-exps (... ...))))
                                    val
                                    (let ([w (opt-wrap-maker 
                                              val contract/info)])
...
(define-rstruct contract/info (contract blame))
(define-rstruct opt-contract/info (contract enforcer id))

;; parents : (listof wrap-parent)
;; vals : hash-table[symbol -o> 
;;         (union (make-unknown proc[field-vals -> any]) any)
;; test : proc[vals-hash-table -> boolean]
(define-rstruct synth-info (parents vals test))

(define-rstruct unknown (proc))

(define secret (gensym))
(define (synthesized-value wrap key)
  (unless (wrap-parent-predicate wrap)
    (error 'synthesized-value 
           "expected struct value with contract as first argument, got ~a"
           wrap))
  (let ([ans (hash-ref (synth-info-vals (wrap-parent-get wrap 0))
                       key
                       secret)])
    (when (eq? ans secret)
      (error 'synthesized-value "the key ~a is not mapped in ~a" key wrap))
    ans))
...
(define (check-synth-info-test stct synth-info contract/info)
  (unless ((synth-info-test synth-info) (synth-info-vals synth-info))
    (raise-blame-error
     (contract/info-blame contract/info)
     stct
     "failed `and' clause, got ~a" stct)))

(define-values (evaluate-attr-prop evaluate-attr-prop-predicate 
                                   evaluate-attr-prop-accessor)
  (make-struct-type-property 'evaluate-attr-prop))

(define-values (wrap-parent-type wrap-parent-maker wrap-parent-predicate 
                                 wrap-parent-get wrap-parent-set)
