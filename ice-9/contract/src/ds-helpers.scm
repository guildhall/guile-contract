(define-module (ice-9 contract src ds-helpers)
  ...
  (let* ([field-names 
    ...
    (let loop ([clauses (syntax->list clauses)]
      ...
      (cond
        ...
        [else 
         (let ([clause (car clauses)])
           (syntax-case clause (and where)
             [where
              (build-clauses/where 
               name stx (cdr clauses) field-names (reverse maker-args))]
             [and
              (build-clauses/and 
               name stx (cdr clauses) '() '() (reverse maker-args))]
             [else
                ...
                (syntax-case clause ()
                  ...
                  [(id (x ...) ctc-exp)
                   (begin
                     (unless (identifier? (syntax id))
                       (raise-syntax-error 
                        name "expected identifier" stx (syntax id)))
                     (for-each (λ (x) (unless (identifier? x)
                                        (raise-syntax-error 
                                         name "expected identifier" stx x)))
                  ...
                  [(id ctc-exp)
                   ...
                   (loop (cdr clauses)
                         (cdr ac-ids)
                         (cons (car ac-ids) prior-ac-ids)
                         (cons #`(#,coerce-contract '#,name ctc-exp) 
                               maker-args))]
                  [(id ctc-exp)
                   (raise-syntax-error name "expected identifier" stx 
                                       (syntax id))]
                  [_
                   (raise-syntax-error name "expected name/identifier binding" 
                                       stx clause)]))]))]))))
  ...
  (with-syntax ([(field-names ...) field-names])
    (let loop ([clauses clauses]
      ...
      (cond
        ...
        [else 
         (let ([clause (car clauses)])
           (syntax-case clause (and)
             [and (build-clauses/and 
                   name stx (cdr clauses) vars procs maker-args)]
             [(id exp) 
              (identifier? (syntax id))
              (loop (cdr clauses)
                    (cons (syntax id) vars)
             ...
             [(id exp)
              (raise-syntax-error 
               name "expected an identifier" stx (syntax id))]
             [_ 
              (raise-syntax-error 
               name "expected an identifier and an expression" 
               stx clause)]))]))))



(define (build-enforcer-clauses 
         opt/i opt/info name stx clauses f-x/vals f-xs/vals
         helper-id helper-info helper-freev)  
  (define (opt/enforcer-clause id stx)
    (syntax-case stx ()
      [(f arg ...)
       ;; we need to override the default optimization of recursive 
       ;; calls to use our helper
       (and (opt/info-recf opt/info) 
...
(define (build-clauses/and name stx clauses synth-names synth-procs maker-args)
  (unless (pair? clauses)
    (raise-syntax-error 
     name "expected an expression after `and' keyword" stx))
  (unless (null? (cdr clauses))
    (raise-syntax-error 
     name "expected only one expression after `and' keyword"
     stx (cadr clauses)))
        ...
        [else (let* ([ac-id (car prior-ac-ids)]
                     ...
                     [id-used 
                      (ormap (λ (used-field-id) 
                               (and (eq? (syntax-e field-name) 
                                         (syntax-e used-field-id))
                                    used-field-id))
                             used-field-ids)])
                (if id-used
...
(define (sort-wrt name stx ids current-order-field-names 
                  desired-order-field-names)
  (let ([id/user-specs (map cons ids current-order-field-names)]
    ...
    (let loop ([i 0]
      ...
      (unless (null? orig-field-names) 
        (hashq-set! ht (syntax-e (car orig-field-names)) i)
  ...
  (syntax-case stx ()
    ...
    [(_ [id exp] ...)
     ...
     (raise-syntax-error 'struct/dc 
                         (format #f "expected ~a clauses, but found ~a"
                                 field-count
                                 (length (syntax->list (syntax (id ...)))))
                         stx)]
    [(_ [id exp] ...)
     (for-each
      (λ (id) (unless (identifier? id) (raise-syntax-error 
                                        'struct/dc 
                                        "expected identifier" stx id)))
      (syntax->list (syntax (id ...))))]))

(define-rstruct contract-struct-transformer (info proc)
