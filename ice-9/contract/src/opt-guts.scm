(define-module (ice-9 contract src opt-guts)
  ...
  #:export (get-opter reg-opter! opter
                      interleave-lifts
...
#|
#lang racket/base
(require syntax/private/boundmap ;; needs to be the private one, since the
                                 ;; public one has contracts
...
(define make-module-identifier-mapping make-free-identifier-mapping)
(define module-identifier-mapping-get  free-identifier-mapping-get)
(define module-identifier-mapping-put!  free-identifier-mapping-put!)
...
(define (opter ctc)
  (if (identifier? ctc)
      (get-opter ctc)
      (rerror 'opter "the argument must be a bound identifier, got ~a" ctc)))
...
(define (interleave-lifts vars sexps)
  (if (= (length vars) (length sexps))
      ...
      (rerror 'interleave-lifts 
              "expected lists of equal length, got ~a and ~a" vars sexps)))


;; struct for color-keeping across opters
(define-rstruct opt/info
...
(define (opt/info-swap-blame info)
  (make-opt/info
   (opt/info-contract info)
   (opt/info-val      info)
   (opt/info-blame-id info)
   (not (opt/info-swap-blame? info))
   (opt/info-free-vars info)
   (opt/info-recf info)
   (opt/info-base-pred info)
   (opt/info-this info)
   (opt/info-that info)))
...
(define-rstruct stronger-rib (this-var that-var save-id stronger-exp))

(export new-stronger-var)
...
;; values that are lifted are also saved in the wrapper to make sure that the 
;; rhs's are evaluated at the right time.
...
(define (opt/unknown opt/i opt/info uctc)
  (let* ((lift-var         (car (generate-temporaries (syntax (lift)))))
         ...
         (partial-flat-var (car (generate-temporaries 
                                 (syntax (partial-flat))))))
    (values
              ...
              (syntax 
               (if (flat-contract? lift-var)
                   ...
                   (lambda (x) 
                     (rerror 
                      'opt/unknown 
                      "flat called on an unknown that had no flat pred ~s ~s"
                      lift-var
                      x)))))))
...
(export lift/binding lift/effect empty-lifts bind-lifts bind-superlifts 
        lifts-to-save)
