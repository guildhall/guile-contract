(define-module (ice-9 contract src opt)
  ...
  #:export (opt/c define-opt/c define/opter opt-stronger-vars-ref
                  opt/direct
                  begin-lifted))
...
;;         syntax syntax-list syntax-list syntax-list (union syntax #f) 
;;         (union syntax #f) syntax
;;         
;;
;; It takes in an identifier for pos, neg, and the original syntax. An 
;; identifier
;; that can be used to call the opt/i function is also implicitly passed into
;; every opter. A list of free-variables is implicitly passed if the calling 
;; context
...
;;  - super-lifted variables: functions or the such defined at the toplevel of 
;;                            the calling context of the opt routine.
...
;;    else #f
;;    This is used in conjunction with optimizing flat contracts into one 
;;    boolean expression when optimizing or/c.
      ...
      [(_ (for opt/i opt/info stx) expr ...)
       (if (identifier? #'for)
           #'(begin
               ...
               (void))
           (rerror 'define/opter 
                   (string-append "expected opter name to be an identifier, "
                                  "got ~.s" (syntax-e #'for))))])))
...
;;
(define (opt/recursive-call opt/info stx)
  (values
   (with-syntax ((stx stx)
...
(define (make-stronger strongers)
  ...
  (syntax-case stx ()
    ...
    [(_ e (opt-recursive-args ...))
     (let*-values ([(info) (make-opt/info 
                            ...
                            #'this
                            #'that)]
                   [(next lifts superlifts partials _ __ stronger-ribs) 
                    (opt/i info #'e)])
              ...
              (λ (ctc)
                (λ (blame)
                  #,(if (syntax-parameter-value #'define/opt-recursive-fn)
                        (with-syntax 
                            ([f (syntax-parameter-value 
                                 #'define/opt-recursive-fn)])
                          (bind-superlifts
                           (cons
                            (cons (syntax-parameter-value 
                                   #'define/opt-recursive-fn)
...
(define-syntax begin-lifted 
  (lambda (stx)
    (syntax-case stx ()
      [(_ expr)
       (syntax-local-lift-expression #'expr)])))

(define-syntax define/opt-recursive-fn #f)

(define-syntax define-opt/c 
  (lambda (stx)
  (syntax-case stx ()
    [(_ (id args ...) body)
     #'(define (id args ...)
         (fluid-let-syntax ([define/opt-recursive-fn #'id])
                           (opt/c body (args ...))))])))
...
(define-rstruct opt-contract (proj orig-ctc stronger stronger-vars stamp)
  #:property orig-ctc-prop (λ (ctc) ((opt-contract-orig-ctc ctc)))
  #:property prop:contract
  (build-contract-property
   #:projection (λ (ctc) ((opt-contract-proj ctc) ctc))
   ;; I think provide/contract and contract calls this, so we are in effect
   ;; allocating the original once
