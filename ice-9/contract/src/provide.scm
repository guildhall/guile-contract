...
(use-modules ((ice-9 contract src helpers)
              #:renamer (symbol-prefix-proc 'a:)))

(define-syntax todo
  (syntax-rules () 
    ((_ x)
     (define (x . l) 
       (error (format #f "~a not implemented" 'x))
       #f))))

;;TODO (stis)
(begin
  (todo resolved-module-path-name)
  (todo syntax-source-module)
  (todo module-path-index-split)
  (todo module-path-index-resolve)
  (todo syntax-local-context)
  (todo path-string?)
  (todo path->directory-relative-string)
  (todo syntax-line)
  (todo syntax-column)
  (todo syntax-position)
  (todo syntax-span)
...
(define make-applicable-struct-info
  (let ((reff #f))
    (let-values ([(struct: make- ? ref set!-)
                  (make-struct-type 
                   'self-ctor-struct-info struct:struct-info
                   1 0 #f
                   (list (cons prop:procedure
                               (lambda (v stx)
                                 (self-ctor-transformer ((reff v 0)) stx))))
                   (fluid-ref current-inspector) #f '(0))])
      (set! reff ref)
      make-)))
...
(define (first-requiring-module id self)
  ...
  ;; we've provided must have been required.  If the second returned value is 
  ;; #f,
...
(define (make-provide/contract-transformer contract-id id external-id 
                                           pos-module-source)
     ...
     (λ (stx)
       (if #t
           ...
           (let ([key #f])
             ...
             (let ([lifted-id
                    (or (hashq-ref saved-id-table key #f)
                        ...
                        (with-syntax ([contract-id contract-id]
                                      ...
                                      [loc-id 
                                       (identifier-prune-to-source-module id)])
                          (let ([srcloc-code 
                                 (with-syntax 
                                      ([src                      
                                       (cond
                            ...
                            (syntax-local-introduce
                             (syntax-local-lift-expression
                              #`(contract contract-id
                                          id
                                          pos-module-source
                                          (first-requiring-module 
                                           #'loc-id (quote-module-path))
                                          'external-id
                                          #,srcloc-code))))))])
               (when key
                 (hashq-set! saved-id-table key lifted-id))
  ...
  (syntax-case provide-stx (struct)
    [(_ p/c-ele ...)
     (let ()
       ...
       (define dups-table (make-hash-table))
         ...
         (let loop ([clauses clauses]
           ...
           [else
            (let ([clause (car clauses)])
              ;; compare raw identifiers for `struct' and `rename' just like 
              ;; provide does
              (syntax-case clause (struct rename) 
                [exists
		 (or (eq? '#:exists (syntax-e #'exists)) 
                     (eq? '#:∃ (syntax-e #'exists)))
                 (cond
                  [(null? (cdr clauses))
                   (raise-syntax-error 
                    ...
                    (format 
                     #f 
                     "expected either a single variable or a sequence of variables to follow ~a, but found nothing"
                     (syntax-e #'exists))
                    provide-stx
                    clause)]
                  [else
                     ...
                     [x
                      ...
                      [else
                       (raise-syntax-error 
                        ...
                        (format
                         #f
                         "expected either a single variable or a sequence of variables to follow ~a"
                         (syntax-e #'exists))
                        provide-stx
                        (cadr clauses))])])]
                ...
                [(rename . _)
                 (raise-syntax-error 
                  'provide/contract 
                  "malformed rename clause" provide-stx clause)]
                ...
                [(struct name . rest)
                 ...
                 (raise-syntax-error 
                  'provide/contract 
                  (string-append "name must be an identifier or two identifiers"
                                 " with parens around them")
                  provide-stx
                  (syntax name))]
                ...
                [(struct name (fields ...))
                 ...
                 (raise-syntax-error 
                  'provide/contract "internal error" provide-stx clause)]
                [(struct name . fields)
                 (raise-syntax-error 'provide/contract
       ...
       ;; build-struct-code : syntax syntax (listof syntax) (listof syntax) 
       ;;                           -> syntax
       ;; constructs the code for a struct clause
       ;; first arg is the original syntax object, for source locations
       (define (build-struct-code stx struct-name-position field-names 
                                  field-contracts)
         (let* ([struct-name (syntax-case struct-name-position ()
                ...
                [parent-struct-count 
                 (if (null? all-parent-struct-count/names)
                     #f
                ...
                [struct-info 
                 (a:lookup-struct-info struct-name-position provide-stx)]
                ...
                [type-is-only-constructor? 
                 (free-identifier=? constructor-id struct-name)]
                [type-is-constructor? #t] 
                ;; I think there's no way to detect when the struct-name 
                ;; binding isn't a constructor
                [is-id-ok?
                 (λ (id i)
                ...
                ;; (listof (union #f identifier))

                [field-contract-ids 
                 (map (λ (field-name field-contract) 
           ...
           (let ([unknown-info
                  (λ (what names)
                    (raise-syntax-error
                     'provide/contract
                     (format #f "cannot determine ~a, found ~s" what names)
                     provide-stx
                     struct-name))])
           ...
           (unless (equal? (length selector-ids)
             ...
             (raise-syntax-error 
              'provide/contract
              (format #f "found ~a field~a in struct, but ~a contract~a"
                      (length selector-ids)
           ...
           (let* ([relative-counts 
                  ...
                  [predicate-name (format #f "~a" (syntax-e predicate-id))]
                  [struct-name 
                   (substring predicate-name 0 
                              (- (string-length predicate-name) 1))])
               ...
               (cond
                 ...
                 [else
                  (let* ([selector-str (car selector-strs)]
                         ...
                         [field-name-is (format #f "~a" (syntax-e field-name))])
                    (unless (equal? field-name-should-be field-name-is)
                      (raise-syntax-error 
                       'provide/contract
                       (format #f "expected field name to be ~a, but found ~a"
                               field-name-should-be
                               field-name-is)
                       provide-stx
                       field-name))
           ...
           (with-syntax ([((selector-codes selector-new-names) ...)
                          (filter
                           (λ (x) x)
                           (map/count                            
                            (λ (selector-id field-contract-id index)
                               (if (is-new-id? index)
                         ...
                         [(mutator-codes/mutator-new-names ...)
                          (map/count 
                           (λ (mutator-id field-contract-id index)
                         ...
                         [(predicate-code predicate-new-name)
                          (code-for-one-id/new-name 
                           stx predicate-id #f (syntax (-> any/c boolean?)) #f)]
                 ...
                 (with-syntax 
                            ...
                            [super-id 
                             (if (boolean? super-id)
                                 ...
                                 (with-syntax ([super-id super-id])
                                   (syntax 
                                    ((syntax-local-certifier) #'super-id))))]
                   ...
                   (syntax
                    (begin
                      ...
                      ;; expanding out the body of the `make-pc-struct-type' 
                      ;; function
                      ;; directly here in the expansion makes this very 
                      ;; expensive at compile time
                      ;; when there are a lot of provide/contract clause using 
                      ;; structs
                      (define -struct:struct-name 
                        (make-pc-struct-type 'struct-name struct:struct-name 
                                             field-contract-ids ...))
                      (provide (rename-out [-struct:struct-name 
                                            struct:struct-name]))))))))))
       ...
       ;; get-field-counts/struct-names : syntax syntax 
       ;;          -> (listof (cons number symbol))
       ;; returns a list of numbers corresponding to the numbers of fields 
       ;; for each of the parent structs
                 ...
                 (cond
                  [(and (not (null? fields))
                   ...
                   (raise-syntax-error 
                    ...
                    struct-name)]
                  [else
                   (cons (cons (length fields) (predicate->struct-name 
                                                provide-stx predicate))
                         (loop (list-ref parent-info 5)))]))]))))
       
       (define (predicate->struct-name orig-stx stx)
         (and stx
              (let ([m (regexp-exec "^(.*)[?]$" 
                                    (format #f "~a" (syntax-e stx)))])
                (cond
                  [m (cadr m)]
                  [else (raise-syntax-error 
                         'contract-base.rkt
                         (string-append "unable to cope with a struct supertype"
                                        " whose predicate doesn't end with `?'")
                         orig-stx)]))))
       ...
       (define (build-selector-contract struct-name predicate-id 
                                        field-contract-id)
       ...
       ;; constructs the contract for a selector
       (define (build-mutator-contract struct-name predicate-id 
                                       field-contract-id)
         (with-syntax ([field-contract-id field-contract-id]
                       [predicate-id predicate-id])
           (syntax (-> predicate-id
       ...
       ;; code-for-one-id/new-name : syntax syntax syntax (union syntax #f) 
       ...
       (define code-for-one-id/new-name
         (case-lambda
           [(stx id reflect-id ctrct user-rename-id) 
            (code-for-one-id/new-name
             stx id reflect-id ctrct user-rename-id #f #t)]
           [(stx id reflect-id ctrct user-rename-id mangle-for-maker?)
            (code-for-one-id/new-name 
             id reflect-id ctrct user-rename-id mangle-for-maker? #t)]
           [(stx id reflect-id ctrct/no-prop user-rename-id mangle-for-maker? 
                 provide?)
            (let ([no-need-to-check-ctrct? 
                   (a:known-good-contract? ctrct/no-prop)]
              ...
              (with-syntax ([id-rename ((if mangle-for-maker? 
                ...
                (with-syntax ([extra-test 
                      ...
                      ([code
                        (syntax-property
                         #`((begin
                              (define pos-module-source 
                               ...
                               #,@(if no-need-to-check-ctrct?
                                      ...
                                      (list #'(define contract-id 
                                                (let ([ex-id ctrct]) 
                                                  ;; let is here to give the 
                                                  ;; right name.
                                                  (verify-contract 
                                                   'provide/contract ex-id))))))
                             (define-syntax id-rename
                               (make-provide/contract-transformer 
                                #'contract-id)
                               (a:update-loc
                           ...
                           #,@(if provide?
                                  (list #`(export (rename-out 
                                                   [id-rename external-name])))
                                  null))
                  ...
                  (syntax-local-lift-module-end-declaration
                   #`(begin 
                       (unless extra-test
                         (contract contract-id id pos-module-source 'ignored 
                                   'id 
                                   (quote-srcloc id)))
                       (void)))
     ...
     (with-syntax ([(bodies ...) 
                    (code-for-each-clause (syntax->list 
                                           (syntax (p/c-ele ...))))])
