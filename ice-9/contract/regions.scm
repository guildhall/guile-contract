(define-module (ice-9 contract regions)
...
(define (quote-srcloc x)
  "-")

(begin
  (todo syntax-local-context)
  (todo set-field-info-mutable?!)
  (todo field-info-mutable?)
  (todo local-expand)
  (todo kernel-form-identifier-list)
  (todo syntax-local-make-definition-context)
  (todo make-syntax-introducer)
  (todo internal-definition-context-apply)
  (todo syntax-local-bind-syntaxes)
  (todo internal-definition-context-seal))
     ...
     ((splicing fv-clause
      ...
      (splicing fvs
        ...
        [pattern (~seq f:fv-clause ...)
                 ...
                 (format #f "duplicate imported name ~a" 
                     (syntax-e (check-duplicate-identifier 
                                (syntax->list #'(var ...)))))]))
        ...
        (let-values ([(name body-expr)
                      (normalize-definition
                       (datum->syntax #'define-stx 
                                      (list* 'define/contract #'name+arg-list
                                             #'body0 #'(body ...)))
                       #'lambda* #t #t)])
...
(define-rstruct contract-struct-info (si cons)
...
(define-syntax define-struct/contract 
  (lambda (stx)
  (define-rstruct field-info (stx ctc [mutable? #:mutable] auto?))
  (define-rstruct s-info (auto-value-stx transparent? def-stxs? def-vals?))
      ...
      (list* 
       ...
       (apply append
              (for/list ([finfo field-infos])
                (let ([field-str 
                       (symbol->string (syntax-e (field-info-stx finfo)))])
                  (cons (datum->syntax 
                         name
                         (string->symbol
     ...
     (let ([fields (syntax->list #'(field ...))])
       ...
       (let* ([field-infos (map check-field fields (syntax->list #'(ctc ...)))]
                 ...
                 (if (identifier? super)
                     (let ([v (syntax-local-value 
                       ...
                       (unless (struct-info? v)
                         (raise-syntax-error 
                          #f 
                          "identifier is not bound to a structure type"
                          stx super))
              ...
              [super-muts 
               (let ([super (cadr names)])
                 (if (identifier? super)
                     (let ([v (syntax-local-value super 
                                                  (lambda () #f))])
                       (unless (struct-info? v)
                         (raise-syntax-error 
                          #f 
         ...
         (let-values ([(non-auto-fields auto-fields)
           ...
           (with-syntax ([ctc-bindings
                         ...
                         [(super-contract ...) (for/list 
                                                ([i (in-list super-refs)])
             ...
             (with-syntax ([(stx-def ...)
                 ...
                 (syntax
                   (begin
                     ...
                     stx-def ...
                     (define (guard super-field ... non-auto-name ... 
                                    struct-name)
...
(define (make-contracted-id-transformer id contract-stx pos-blame-stx 
                                        neg-blame-stx)
         ...
         (syntax-case stx (set!)
           [(set! i arg)
            (begin
              (set! first #f)
              (quasisyntax
               (set! id (contract ctc arg neg pos (quote id) 
                                  (quote-srcloc id)))))]
           [(f arg ...)
            (begin
              #;(when first
                (error (format #f "variable ~a has not been defined" 
                               (syntax->datum #'f))))
              (quasisyntax
...
;;TODO, implement a valid local-expand 
(define-syntax with-contract-helper 
  (lambda (stx)
  (syntax-case stx ()
    [(_ ())
     #'(begin)]
...
(define-syntax with-contract 
  (lambda (stx)
    (with-syntax-data
     ((splicing region-clause
      ...
      (class exports-clause
        ...
        [pattern (ec:export-clause ...)
                 ...
                 #:fail-when (check-duplicate-identifier 
                              (syntax->list #'(var ...)))
                 (format #f "duplicate exported name ~a" 
                         (syntax-e (check-duplicate-identifier 
                                    (syntax->list #'(var ...)))))])

      (splicing results-clause
        #:attributes ([ctc 1])
     ...
     (syntax-parse stx
       [(_ (~optional :region-clause #:defaults ([region #'region]))
           blame:id rc:results-clause fv:fvs body ...)
        (if #f #;(not (eq? (syntax-local-context) 'expression))
            (quasisyntax (%expression #,stx))
         (let*-values ([(intdef) 1 #;(syntax-local-make-definition-context)]
                       [(ctx) (list (gensym "intdef"))]
                       [(cid-marker) 1 #;(make-syntax-introducer)]
                       [(free-vars free-ctcs)
                        (values (syntax->list #'(fv.var ...))
                                (syntax->list #'(fv.ctc ...)))])
           (define (add-context stx) stx
           ...
           (with-syntax ([blame-stx #''(region blame)]
                         ...
                         [(free-var ...) free-vars]
                         [(free-var-id ...) 
                          (generate-temporaries free-vars)
                          #;(add-context #`#,free-vars)]
                         [(free-ctc-id ...) 
                          (generate-temporaries free-vars)
                          #;(map cid-marker free-vars)]
                         [(free-ctc ...) 
                          (map (λ (c v)
             ...
             (with-syntax 
                 ([new-stx 
                   (add-context #'(fluid-let-syntax
                                   ([current-contract-region 
                                     (λ (stx) #'blame-stx)])
                                   (let-values ([(res ...) (let () body ...)])
                                     (values (contract 
                                              (verify-contract 
                                               'with-contract rc.ctc)
                                              res
                                              blame-stx
                                              blame-id) ...))))])
               (quasisyntax
       ...
       [(_ (~optional :region-clause #:defaults ([region #'region])) 
           blame:id ec:exports-clause fv:fvs body ...)
        #;(when (memq (syntax-local-context) '(expression module-begin))
        (raise-syntax-error 'with-contract
                              "not used in definition context"
                              stx))
        (let*-values ([(intdef) 1 #;(syntax-local-make-definition-context)]
          ...
          (with-syntax ([blame-stx #''(region blame)]
                        ...
                        [(f-ctcs ...) free-ctcs]
                        [(ctc-id ...) 
                         (generate-temporaries protected)
                         #;(map cid-marker protected)]
                        [(prs ...) protections]
                        [(ctc ...) (map (λ (c v)
                                           (syntax-property 
                                            (add-context c) 'inferred-name v))
                                        protections
                                        protected)]
                        [(p ...) protected]
                        [(marked-p ...) 
                         (generate-temporaries protected)
                         #;(add-context #`#,protected)])

            (with-syntax ([new-stx #'(let () body ...)
                                   #;(add-context 
                                    #'(fluid-let-syntax 
                                       ([current-contract-region 
                                         (λ (stx) #'blame-stx)])
              ...
              (quasisyntax
               (begin
                 (define blame-id
                   (current-contract-region))
                 ;;Protected global contracted vars
                 
                 (define-values (ctc-id ...)
                   (values (verify-contract 'with-contract prs) ...))
                 ...
                 ;;Local contracted vars
                 
                 (define-values (free-ctc-id ...)
                   (values (verify-contract 'with-contract f-ctcs) ...))
                 
                 (define free-var-id #f) ...
